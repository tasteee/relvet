...
export const createMidiEngine = (): MidiEngineT => {
	...
	const updateLivePlayback = just.debounce(150, () => {
		...
		store.progression.steps.forEach((step, stepIndex) => {
			...
			const stepStartBeats = accumulatedBeats
			const stepStartTicks = theory.getBeatsTicks(stepStartBeats)
	...
	const scheduleMidiPlayback = () => {
		if (!$output.isOutputEnabled) return
		const instrument = $output.instrument
		const midiOutput = $output.midiOutput
		...
		const isOutputMidi = $output.target === 'midi'
		const isOutputInstrument = $output.target === 'instrument'
		...
		if (isOutputInstrument) {
			const instrumentKey = $output.instrumentName as InstrumentName
			if (!instrumentKey || !$output.instruments[instrumentKey]) return
