 ...
 */
function generateChordsForRoot(root: string, scaleNotes: string[]): ChordT[] {
	const validChords: ChordT[] = []
		...
		try {
			...
			if (chord.notes && chord.notes.length > 0) {
				if (chordFitsInScale(chord.notes, scaleNotes)) {
					const base = {
						tonic: root,
						color: theory.getTonicColor(root),
						notes: chord.notes,
						octave: 0,
					...
					const adjustedNotes = getAdjustedNotes({ baseOctave, chord: base })
					validChords.push({ ...base, adjustedNotes })
				}
			}
		} catch (error) {
...
export function getVastScaleChords(scaleInput: string): ChordT[] {
	...
	const uniqueChords = allChords.filter((chord, index, self) => index === self.findIndex((c) => c.symbol === chord.symbol))

	uniqueChords.sort((a, b) => {
		const rootA = a.symbol.match(/^[A-G][#b]?/)?.[0] || ''
		const rootB = b.symbol.match(/^[A-G][#b]?/)?.[0] || ''
...
export function getChordStatistics(chords: ChordT[]): {
	totalChords: number
	...
	chords.forEach((chord) => {
		const root = chord.symbol.match(/^[A-G][#b]?/)?.[0] || 'Unknown'
		const type = chord.symbol.replace(/^[A-G][#b]?/, '') || 'M'
