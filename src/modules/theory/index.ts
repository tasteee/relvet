...
const getChord = memoize((chordName: string) => {
	...
	const color = appConfig.rootNoteColors[chord.tonic] || 'gray'
	return { ...chord, color }
})
...
const getRepeatedScaleNotes = (scaleNotes: string[]) => {
	return [...scaleNotes, ...scaleNotes, ...scaleNotes, ...scaleNotes, ...scaleNotes, ...scaleNotes, ...scaleNotes, ...scaleNotes]
}
...
import '#/constants/theory/scale-chords'
import { appConfig } from '#/constants/appConfig'

const getTonicColor = (tonic: string) => {
	return appConfig.rootNoteColors[tonic] || 'gray'
}
