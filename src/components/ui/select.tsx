...
const LABEL_TW_CLASSES = 'text-foreground z-10 text-xs font-medium group-has-disabled:opacity-50 inline'

const OverlayLabel = (props) => {
	const classes = classNames('selectLabel', LABEL_TW_CLASSES, props.className)
...
function SelectTrigger({ className, children, ...props }: React.ComponentProps<typeof SelectPrimitive.Trigger>) {
	return (
		<SelectPrimitive.Trigger
			...
			className={cn(
				'border-input text-foreground data-[placeholder]:text-muted-foreground focus-visible:border-ring focus-visible:ring-ring/50 aria-invalid:ring-destructive/20 dark:aria-invalid:ring-destructive/40 aria-invalid:border-destructive flex h-9 w-full items-center justify-between gap-2 rounded-xs border bg-transparent px-3 pb-2 text-sm shadow-xs transition-[color,box-shadow] outline-none focus-visible:ring-[3px] disabled:cursor-not-allowed disabled:opacity-50 *:data-[slot=select-value]:flex *:data-[slot=select-value]:items-center *:data-[slot=select-value]:gap-2 [&_svg]:pointer-events-none [&_svg]:shrink-0 [&>span]:line-clamp-1',
				className
			)}
			style={{ paddingTop: '18px', paddingBottom: 14 }}
			{...props}
		>
			{children}
