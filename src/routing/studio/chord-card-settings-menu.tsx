...
export const SettingsDropdown = (props) => {
	...
	const handleOctaveChange = (value) => {
		props.chord.setOctave(value)
	}

	const handleInversionChange = (value) => {
		props.chord.setInversion(value)
	}

	const handleVoicingChange = (value) => {
		props.chord.setVoicing(value)
	}

	const handleBassNoteChange = (value) => {
		props.chord.setBassNote(value)
	}
		...
		<DropdownMenu.Root>
			...
			<DropdownMenu.Content className='min-w-[220px] rounded-xs'>
				<Flex.Column p='2' gap='2' className='innerColumn'>
					...
					{/* Bass Note Row */}
					<Flex.Row className='BassNoteMenuRow' align='center' justify='between' aria-label='Bass Note'>
